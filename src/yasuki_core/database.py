...
def query_clans_filtered(text_query: str = "", filter_options: dict | None = None) -> list[str]:
    ...
    where_clause, params = build_card_filter(text_query, options)
    sql = (
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            results = sorted(
                {
                    CLAN_CANONICAL.get(row["clan"].lower(), row["clan"])
                    for row in cur.fetchall()
...
def query_cards_filtered(
    ...
    """
    where_clause, params = build_card_filter(text_query, filter_options)
    select_sql, select_params = _card_select(active_format_from_options(filter_options))
