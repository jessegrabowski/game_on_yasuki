...
or it sits on a proxy in a seat's rulebook zone as Cycle, Legacy and Inheritance do. Each is a
player ability, so it plays no card, takes no keyword from the card it sits on, and is not announced
as printed. A keyword ability that leaves `from_rulebook` unset is refused when it is built.
