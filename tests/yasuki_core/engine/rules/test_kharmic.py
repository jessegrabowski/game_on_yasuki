...
def test_both_kharmic_abilities_are_offered_in_the_action_phase():
    ...
    assert KharmicDraw() in offered
    assert KharmicRefill() in offered
...
def test_kharmic_is_not_offered_outside_the_action_phase():
    ...
    assert KharmicDraw() not in offered and KharmicRefill() not in offered
...
@pytest.mark.parametrize(
    ("action", "kwargs"),
    [(KharmicDraw(), {"hand_kharmic": 0}), (KharmicRefill(), {"province_kharmic": 0})],
...
def test_a_face_down_province_card_cannot_be_spent():
    ...
    assert KharmicDraw() in offered
    assert KharmicRefill() not in offered
...
def test_the_fate_form_discards_a_kharmic_card_and_draws():
    ...
    _pay(session)
    session.submit(PlayerId.P1, DecisionResponse(("P1-k0",)))
...
def test_the_dynasty_form_discards_from_a_province_and_refills_it_face_up():
    ...
    session.act(PlayerId.P1, KharmicRefill())
    _pay(session)
    session.submit(PlayerId.P1, DecisionResponse(("P1-pk0",)))
...
def test_kharmic_is_repeatable_within_one_turn():
    ...
    assert KharmicDraw() in session.legal_actions(PlayerId.P1)
...
def test_the_inactive_seat_may_take_kharmic_in_the_opponents_action_phase():
    ...
    assert KharmicDraw() in session.legal_actions(PlayerId.P2)
...
def test_paying_on_the_opponents_turn_leaves_the_producer_bowed_into_your_own():
    ...
    session = EngineSession.start(state, PlayerId.P1)
    session.act(PlayerId.P1, Pass())
    session.act(PlayerId.P2, KharmicDraw())
    _pay(session, PlayerId.P2)
    session.submit(PlayerId.P2, DecisionResponse(("P2-k0",)))
