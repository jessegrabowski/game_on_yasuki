...
def test_every_effect_takes_the_game_when_asked_whether_it_is_interruptible():
    ...
    asked = inspect.signature(Effect.is_interruptible)
    wrong = [
        cls.__name__ for cls in _effect_types() if inspect.signature(cls.is_interruptible) != asked
    ]
    assert wrong == []
