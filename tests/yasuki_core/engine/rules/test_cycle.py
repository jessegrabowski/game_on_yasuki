...
from yasuki_core.engine.rules.vocabulary.game_events import ActionResolved, Revealed
...
def _cycle(session: EngineSession, seat: PlayerId = P1) -> ActivateAbility:
    return ActivateAbility(rulebook_proxy(session.game, seat, CYCLE_PROXY_ID).id, CYCLE)
...
def test_cycle_is_offered_on_a_seats_opening_turn():
    session = _session()
    assert _cycle(session) in session.legal_actions(P1)
...
def test_cycle_is_not_offered_once_the_first_turn_has_passed():
    ...
    assert _cycle(session) not in session.legal_actions(P1)
...
def test_the_second_seat_opens_on_turn_two():
    ...
    assert _cycle(session, P2) in session.legal_actions(P2)
...
def test_cycle_is_not_offered_with_nothing_face_up_to_put_back():
    ...
    session = _session(provinces=0)
    assert _cycle(session) not in session.legal_actions(P1)


def test_cycle_is_not_offered_again_once_taken():
    session = _session()
    session.act(P1, _cycle(session))
...
def test_each_seat_is_dealt_its_own_cycle_proxy():
    session = _session()

    proxies = [rulebook_proxy(session.game, seat, CYCLE_PROXY_ID) for seat in (P1, P2)]
    assert [proxy.owner for proxy in proxies] == [P1, P2]


def test_an_arc_without_cycle_deals_no_cycle_proxy(monkeypatch):
    monkeypatch.setattr(ruleset, "ACTIVE", ruleset.IMPERIAL)
    session = _session()

    assert rulebook_proxy(session.game, P1, CYCLE_PROXY_ID) is None


def test_cycle_resolves_as_a_rulebook_ability(reacting):
    resolved = []
    reacting(ActionResolved, "cycle_probe", lambda ctx: resolved.append(ctx.event) or [])
    session = _session()
    put_in_play(session.game, holding("P1-eyes", owner=P1, printed_id="cycle_probe"))
    session.act(P1, _cycle(session))
...
def test_cancelling_the_pick_leaves_cycle_unspent():
    session = _session()
    session.act(P1, _cycle(session))
