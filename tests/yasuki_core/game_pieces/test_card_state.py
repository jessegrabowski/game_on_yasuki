...
def test_card_bow_and_unbow():
    c = L5RCard.of(CardPrint, id="c1", name="Test", side=Side.FATE)
...
def test_card_face_up_down_and_flip():
    c = L5RCard.of(CardPrint, id="c2", name="Test2", side=Side.DYNASTY)
...
def test_adjust_counter_accumulates_floors_at_zero_and_drops_the_key():
    c = L5RCard.of(CardPrint, id="c4", name="Farm", side=Side.DYNASTY)
    assert c.counters == {}
...
def test_replace_built_cards_do_not_share_counters():
    ...
    front = L5RCard.of(CardPrint, id="sh", name="Kyuden", side=Side.STRONGHOLD)
...
def test_counters_survive_a_face_flip():
    ...
    back = L5RCard.of(CardPrint, id="sh__back", name="Kyuden, Defiled", side=Side.STRONGHOLD)
    card = L5RCard.of(
        CardPrint, id="sh", name="Kyuden", side=Side.STRONGHOLD, back_card_id="sh__back", back=back
    )
...
def test_counters_participate_in_card_equality():
    ...
    plain = L5RCard.of(CardPrint, id="c5", name="Farm", side=Side.DYNASTY)
    tokened = L5RCard.of(CardPrint, id="c5", name="Farm", side=Side.DYNASTY)
    tokened.adjust_counter("wealth", 1)
    assert plain != tokened


def test_card_invert_and_uninvert():
    c = L5RCard.of(CardPrint, id="c3", name="Rot", side=Side.FATE)
