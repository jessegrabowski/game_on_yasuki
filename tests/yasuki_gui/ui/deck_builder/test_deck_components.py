...
from yasuki_gui.ui.deck_builder.deck_components import (
    FilteredCardList,
    DeckCardList,
    extract_card_id,
...
def test_filtered_card_list_set_filter_options(root, mock_repository):
    ...
    # Blank search text resolves to the all: predicate, which rides alongside the legality filter.
    mock_repository.filter_cards.assert_called_with(
        "", {"all": True, "legality": ("Ivory Edition", ["legal"])}
    )
