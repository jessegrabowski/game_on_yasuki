...
def _own_calls(function: ast.FunctionDef) -> set[str]:
    """The names this function calls in its own body, ignoring the bodies of functions nested in
    it — those are separate paths with their own obligations."""
    nested = {
        node
...
def test_every_path_that_starts_a_game_hands_over_to_the_opponent():
    ...
    starters = [fn for fn in _functions() if "start" in _own_calls(fn)]
